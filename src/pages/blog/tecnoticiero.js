...
export default function News({ news }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    Router.push(`/blog/tecnoticiero/?searchTerm=${text}/#notice-position`);
  };
    ...
    <Fragment>
      <div>
        <div className="my-10 text-center">
          <h1 className=" text-2xl md:text-3xl text-center uppercase font-bold text-secundaryBlog">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pb-6 text-center">
          <div className="flex flex-col">
            <div className="text-center">
              ...
              <input
                className="m-6 p-2 border-2 border-primaryBlog rounded-md w-44"
                type="text"
                value={text}
                onChange={handleChange("text")}
              />
              <button className="bg-primaryBlog text-white rounded-md w-28 py-2 align-middle">
                Search
              </button>
            </div>
          </div>
      ...
      <div className="text-center">
        <form onSubmit={handleSubmit} className="space-y-4 pt-16 mx-6">
          ...
          <input
            ...
            className="m-6 p-2 border-2 border-primaryBlog rounded-md w-full mx-auto md:w-60"
          />
        </form>
      </div>
    </Fragment>
      ...
      <Layout>
        ...
        <div className="w-5/6 md:w-2/3 mx-auto">
          ...
          <div className="flex justify-center">
            <button
              className="bg-primaryBlog text-white py-3 px-10 my-4 text-lg uppercase rounded-md"
              onClick={() => router.back()}
            >
              Volver 👈
            </button>
...
News.getInitialProps = async ({ query }) => {
  ...
  try {
    const res = await fetch(
      `https://hn.algolia.com/api/v1/search?query=${query.searchTerm || "bitcoin"
      }`
    );
    news = await res.json();
    console.log(news);
